...
void delayMicroseconds(uint32_t del){
	uint32_t k = 0;
	for(;k < 72000000; k=k+23*1000000/del);
}
/**/
...
void RF24init(SPI_HandleTypeDef *HSPI, int ce, int csn, uint8_t channel) {
  ...
  //Set retries = 15 and timeout = 2250us
  ...
  aux = readRegister(CONFIG) | 1 << MASK_TX_DS | 1 << MASK_MAX_RT;
...
volatile unsigned int RF24fullTX(){
	return(readRegister(FIFO_STATUS) & (1<<TX_FULL));
}

volatile unsigned int RF24fullRX(){
	return(readRegister(FIFO_STATUS) & (1<<RX_FULL));
}
