  ...
  sintabinv, emxArray_creal_T *y)
{
  ...
  for (ihi = 0; ihi < rt; ihi++) {
    r = wwc->data[(n1 + ihi) - 1].re;
    twid_im = wwc->data[(n1 + ihi) - 1].im;
    y->data[ihi].re = r * x->data[idx];
    y->data[ihi].im = twid_im * -x->data[idx];
    idx++;
  }
...
void fft(const emxArray_real_T *x, int n, emxArray_creal_T *y)
{
  ...
  if ((x->size[0] == 0) || (n == 0)) {
    pmax = y->size[0];
    y->size[0] = n;
    emxEnsureCapacity_creal_T1(y, pmax);
    if (n > x->size[0]) {
      nn1m1 = y->size[0];
      pmax = y->size[0];
      y->size[0] = nn1m1;
