 ...
 * Return Type  : void
 */
void ezFFT_api(const mxArray * const prhs[2], int32_T nlhs, const mxArray *plhs
               [4])
{
  ...
  emxInit_real_T(&st, &frequencyAxis, 2, true);
  emxInit_real_T(&st, &amplitudeAxis, 2, true);
  emxInit_real_T(&st, &phaseAxis, 2, true);
  emxInit_real_T(&st, &powerSpectrumAxis, 2, true);
  ...
  ezFFT(b_signal, fs, frequencyAxis, amplitudeAxis, phaseAxis, powerSpectrumAxis);

  /* Marshall function outputs */
  plhs[0] = emlrt_marshallOut(frequencyAxis);
  frequencyAxis->canFreeData = false;
  emxFree_real_T(&st, &frequencyAxis);
  b_signal->canFreeData = false;
  emxFree_real_T(&st, &b_signal);
  if (nlhs > 1) {
    plhs[1] = emlrt_marshallOut(amplitudeAxis);
  }

  amplitudeAxis->canFreeData = false;
  emxFree_real_T(&st, &amplitudeAxis);
  if (nlhs > 2) {
    plhs[2] = emlrt_marshallOut(phaseAxis);
  }

  phaseAxis->canFreeData = false;
  emxFree_real_T(&st, &phaseAxis);
  if (nlhs > 3) {
    plhs[3] = emlrt_marshallOut(powerSpectrumAxis);
  }

  powerSpectrumAxis->canFreeData = false;
  emxFree_real_T(&st, &powerSpectrumAxis);
...
/*
 * File trailer for _coder_ezFFT_api.c
 *
 * [EOF]
 */
